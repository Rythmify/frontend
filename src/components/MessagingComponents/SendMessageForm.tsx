...
export default function SendMessageForm({ conversationId, currentUser }: SendMessageFormProps) {
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      // Use the API response but override body with what the user actually typed,
      // since the mock always returns an empty body.
      setSentMessages(prev => [...prev, { ...res.data, body: value.trim() }]);
      setValue('');
      setEmbed(null);
      setBoxKey(k => k + 1);
    } catch {
