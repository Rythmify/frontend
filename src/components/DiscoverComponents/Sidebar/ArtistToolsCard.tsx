      ...
      <div className={styles.toolsGrid}>
        {primaryTools.map((tool) => (
          <button key={tool.id} className={styles.toolButton}>
            ...
            {/* Label Strip — full width */}
            <div className="w-full pb-1">
              <div className={`${styles.toolLabel} group-hover:bg-purple-700`}>
                <span className="group-hover:hidden">{tool.label}</span>
                <span className="hidden group-hover:block">Upgrade</span>
              </div>
            </div>
          </button>
        ))}
            ...
            {secondaryTools.map((tool) => (
              <button key={tool.id} className={styles.toolButton}>
                ...
                <div className="w-full pb-2">
                  <div
                    className={`${styles.toolLabel} group-hover:bg-purple-700`}
                  >
                    <span className="group-hover:hidden">{tool.label}</span>
                    <span className="hidden group-hover:block">Upgrade</span>
                  </div>
