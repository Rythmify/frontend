...
import { TbRadio, TbArrowsExchange, TbChartBar, TbUpload } from "react-icons/tb";
...
function CommentMarker({ comment: c, ratio }: { comment: TrackComment; ratio: number }) {
  ...
  return (
    <div
      ...
      style={{ position: "absolute", left: `${ratio * 100}%`, top: 0, width: 2, height: 24, background: "#eb4926", cursor: "pointer", zIndex: 6 }}
    >
      {hovered && (
        <div style={{
          position: "absolute", bottom: "calc(100% + 6px)", left: "50%", transform: "translateX(-50%)",
          display: "flex", alignItems: "center", gap: 6,
          background: "#222", color: "#fff", fontSize: 12, padding: "5px 10px",
          borderRadius: 4, zIndex: 9999, boxShadow: "0 2px 8px rgba(0,0,0,0.5)",
          maxWidth: 220, whiteSpace: "nowrap", pointerEvents: "none",
        }}>
          <img src={c.avatarUrl} alt={c.username} style={{ width: 18, height: 18, borderRadius: "50%", flexShrink: 0 }} />
          <span style={{ fontWeight: 600, color: "#eb4926", marginRight: 2 }}>{c.username}</span>
          <span style={{ color: "#ccc", overflow: "hidden", textOverflow: "ellipsis", maxWidth: 100 }}>{c.text}</span>
          <span style={{ color: "#666", fontSize: 10, marginLeft: 4, flexShrink: 0 }}>{fmt(c.timestampSec)}</span>
        </div>
      )}
    </div>
  );
...
function CardWaveform({ track, isActive, onWaveformClick, comments, pendingRatio }: CardWaveformProps) {
  ...
  return (
    <div style={{ position: "relative", width: "100%" }}>
      ...
      >
        ...
        <div style={{ position: "absolute", inset: 0, background: "rgba(255,255,255,0.08)", pointerEvents: "none", opacity: isHover ? 0.15 : 0.5, transition: "opacity 0.1s ease", zIndex: 5, borderRadius: 2 }} />
      ...
      <div data-test="waveform-comments-bar" style={{ position: "relative", height: 24 }}>
        {comments.map((c) => {
          const ratio = durSec > 0 ? c.timestampSec / durSec : 0;
          return (
            <CommentMarker key={c.id} comment={c} ratio={ratio} />
          );
        })}
        {pendingRatio !== null && (
          <div
...
function MoreDropdown({ isOwner, liked, onLike, onAddToNext, onAddToPlaylist, onInsights, onDistribute, onDelete }: {
  isOwner: boolean;
  liked?: boolean;
  onLike?: () => void;
  onAddToNext?: () => void;
  ...
  const ownerItems = [
    { label: liked ? "Unlike" : "Like", icon: <FaHeart size={14} />, action: onLike, danger: false },
    { label: "Add to Next up", icon: <MdQueueMusic size={17} />, action: onAddToNext, danger: false },
    { label: "Add to Playlist", icon: <MdPlaylistAdd size={17} />, action: onAddToPlaylist, danger: false },
    { label: "Your Insights", icon: <TbChartBar size={16} />, action: onInsights, danger: false },
    { label: "Station", icon: <TbRadio size={17} />, action: undefined, danger: false },
    { label: "Distribute", icon: <TbUpload size={15} />, action: onDistribute, danger: false },
    { label: "Delete Track", icon: <LuTrash2 size={15} />, action: onDelete, danger: true },
  ];
  const visitorItems = [
    { label: "Add to Next up", icon: <MdQueueMusic size={17} />, action: onAddToNext, danger: false },
    { label: "Add to Playlist", icon: <MdPlaylistAdd size={17} />, action: onAddToPlaylist, danger: false },
  ...
  return (
    <div ref={ref} style={{ position: "relative" }}>
      <button data-test="track-card-btn-more" style={SC_BTN}
        onClick={() => setOpen((p) => !p)}
      ...
      {open && (
        <div data-test="track-card-more-dropdown" style={{ position: "absolute", top: "calc(100% + 6px)", left: 0, zIndex: 9999, background: "#1e1e1e", border: "1px solid rgba(255,255,255,0.12)", borderRadius: 6, boxShadow: "0 6px 24px rgba(0,0,0,0.7)", minWidth: 190, overflow: "hidden" }}>
          {items.map((item) => (
            <button key={item.label}
              data-test={`track-card-more-item-${item.label.toLowerCase().replace(/\s+/g, "-")}`}
              onClick={() => { item.action?.(); setOpen(false); }}
              style={{ display: "flex", alignItems: "center", gap: 12, width: "100%", textAlign: "left", padding: "11px 16px", background: "transparent", border: "none", color: item.danger ? "#ff4444" : "#fff", fontSize: 14, cursor: "pointer", transition: "background 0.1s" }}
              onMouseEnter={(e) => { (e.currentTarget as HTMLElement).style.background = "rgba(255,255,255,0.06)"; }}
              onMouseLeave={(e) => { (e.currentTarget as HTMLElement).style.background = "transparent"; }}>
              <span style={{ color: item.danger ? "#ff4444" : "#aaa", display: "flex", alignItems: "center" }}>{item.icon}</span>
              {item.label}
            </button>
          ))}
        </div>
...
export default function TrackCard({
  ...
  const { currentTrack, isPlaying, setTrack, togglePlay, duration } = usePlayerStore();
  ...
  const [scrollingComment, setScrollingComment] = useState<TrackComment | null>(null);
  ...
  useEffect(() => {
    if (!isActive || isOwner || !localComments.length) { setScrollingComment(null); return; }
    const match = [...localComments]
  ...
  const handleCommentSubmit = () => {
    ...
    const durSec = isActive && duration > 0 ? duration : parseDur(track.duration);
    const tsSec = (commentRatio ?? 0) * durSec;
    const c: TrackComment = {
      id: Date.now(),
      userId: user?.id ?? "anon",
      username: user?.displayName ?? user?.username ?? "You",
      avatarUrl: user?.avatar ?? `https://picsum.photos/seed/${user?.username ?? "u"}/40/40`,
  ...
  const handlePlayPause = () => { if (isActive) togglePlay(); else setTrack(track); };

  const handleWaveformClick = useCallback((ratio: number) => {
    setCommentRatio(ratio);
  ...
  return (
    <>
      {showShare && <SharePopup track={track} onClose={() => setShowShare(false)} />}

      <div data-test="track-card"
        style={{ display: "flex", gap: 16, padding: "20px 0", borderBottom: "1px solid rgba(255,255,255,0.06)" }}>

        {/* Album art */}
        <div data-test="track-card-cover" style={{ width: 130, height: 130, flexShrink: 0, borderRadius: 2, overflow: "hidden", background: "#1a1a1a" }}>
          {track.coverUrl
        ...
        <div style={{ flex: 1, minWidth: 0, display: "flex", flexDirection: "column", gap: 6 }}>
          ...
          <div style={{ display: "flex", alignItems: "flex-start", gap: 10 }}>
            <button data-test="track-card-play-btn" onClick={handlePlayPause}
              style={{ width: 36, height: 36, borderRadius: "50%", background: "#fff", border: "none", cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0, marginTop: 2, transition: "background 0.15s" }}
              onMouseEnter={(e) => ((e.currentTarget as HTMLElement).style.background = "#e0e0e0")}
              onMouseLeave={(e) => ((e.currentTarget as HTMLElement).style.background = "#fff")}>
              {cardIsPlaying ? <FaPause style={{ color: "#111", fontSize: 12 }} /> : <FaPlay style={{ color: "#111", fontSize: 12, marginLeft: 2 }} />}
            </button>

            <div style={{ flex: 1, minWidth: 0 }}>
              <div style={{ fontSize: 13, color: "#999", marginBottom: 2, display: "flex", alignItems: "center", gap: 5, flexWrap: "wrap" }}>
                <Link data-test="track-card-artist-link" to={`/${track.artistUsername}`} style={{ color: "inherit", textDecoration: "none", fontWeight: 600 }}
                ...
                {repostedBy && (
                  <>
                    ...
                    <Link data-test="track-card-reposted-by-link" to={`/${repostedBy}`} style={{ color: "inherit", textDecoration: "none", fontWeight: 500 }}
                      onMouseEnter={(e) => ((e.currentTarget as HTMLElement).style.color = "#fff")}
                      onMouseLeave={(e) => ((e.currentTarget as HTMLElement).style.color = "")}>
                      {repostedBy}
                    </Link>
              ...
              <Link data-test="track-card-title-link" to={`/${track.artistUsername}/${track.trackSlug ?? ""}`}
            ...
            <div style={{ display: "flex", flexDirection: "column", alignItems: "flex-end", gap: 6, flexShrink: 0 }}>
              <span data-test="track-card-posted-at" style={{ fontSize: 12, color: "#999", whiteSpace: "nowrap" }}>{track.postedAt}</span>
              {track.genre && (
                <span data-test="track-card-genre" style={{ background: "#1c1c1c", border: "1px solid rgba(255,255,255,0.2)", color: "#e5e7eb", fontSize: 11, fontWeight: 600, padding: "3px 12px", borderRadius: 99, whiteSpace: "nowrap" }}>
                  # {track.genre}
                </span>
              )}
            </div>
          ...
          {!isOwner && scrollingComment && cardIsPlaying && (
            <div data-test="track-card-scrolling-comment" style={{ display: "flex", alignItems: "center", gap: 8, fontSize: 12, color: "#ccc", overflow: "hidden", whiteSpace: "nowrap" }}>
              <img src={scrollingComment.avatarUrl} alt={scrollingComment.username} style={{ width: 16, height: 16, borderRadius: "50%", flexShrink: 0 }} />
              <span style={{ color: "#eb4926", fontWeight: 600 }}>{scrollingComment.username}:</span>
              <span style={{ overflow: "hidden", textOverflow: "ellipsis" }}>{scrollingComment.text}</span>
            </div>
          )}
          ...
          {commentRatio !== null && (
            <div data-test="track-card-comment-bar" style={{ display: "flex", alignItems: "center", gap: 10, marginTop: 2 }}>
              <img
                src={user?.avatar ?? `https://picsum.photos/seed/${user?.username ?? "u"}/40/40`}
                alt="You"
              ...
              <input
                ...
                onKeyDown={(e) => {
                  ...
                  if (e.key === "Escape") { setCommentText(""); setCommentRatio(null); }
                }}
              ...
              <button
                ...
                onClick={() => { setCommentText(""); setCommentRatio(null); }}
                style={{ ...SC_BTN, padding: "0 10px", background: "#222", color: "#999" }}
                onMouseEnter={(e) => { (e.currentTarget as HTMLElement).style.background = "#333"; }}
                onMouseLeave={(e) => { (e.currentTarget as HTMLElement).style.background = "#222"; }}
              >
                ✕
          ...
          <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginTop: 4 }}>
            ...
            <div style={{ display: "flex", alignItems: "center", gap: 14 }}>
              <span data-test="track-card-play-count" style={{ display: "flex", alignItems: "center", gap: 4, color: "#999", fontSize: 13 }}>
                <FaPlayCount size={10} />{fmtN(track.playCount)}
              </span>
              <span data-test="track-card-comment-count" style={{ display: "flex", alignItems: "center", gap: 4, color: "#999", fontSize: 13 }}>
                <MdComment size={13} />{fmtN(track.commentCount)}
              </span>
            </div>
          </div>
        </div>
